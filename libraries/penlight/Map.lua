--- A Map class.
--
--    > Map = require 'libraries.penlight.Map'
--    > m = Map{one=1,two=2}
...
local tablex = require 'libraries.penlight.tablex'
local utils = require 'libraries.penlight.utils'
local stdmt = utils.stdmt
local deepcompare = tablex.deepcompare

local pretty_write = require 'libraries.penlight.pretty' . write
local Map = stdmt.Map
local Set = stdmt.Set

local class = require 'libraries.penlight.class'
...
local function makelist(t)
    return setmetatable(t, require('libraries.penlight.List'))
end
